...
// النسخة الأصلية (100% كاملة) + دعم PWA Offline + الداش بورد كاملة + المعلم التفاعلي 3D
// ============================================================================
...
function showCustomAlert(message, type = 'error') {
    ...
    overlay.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.8); z-index:99999999; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; direction:rtl; font-family: "Cairo", sans-serif; backdrop-filter: blur(5px); padding:20px; overflow-y:auto;';
    
    overlay.innerHTML = `
        <div style="background:#ffffff; width:100%; max-width:400px; border-radius:16px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); text-align:center; padding:30px 20px; border-top: 5px solid ${titleColor}; animation: scaleInAlert 0.3s ease; margin: auto; max-height: 95vh; overflow-y: auto;">
            ${icon}
...
function showCustomConfirm(message, callback) {
    ...
    overlay.innerHTML = `
        <div style="background:#ffffff; width:100%; max-width:400px; border-radius:16px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); text-align:center; padding:30px 20px; border-top: 5px solid #f59e0b; animation: scaleInAlert 0.3s ease; margin: auto; max-height: 95vh; overflow-y: auto;">
...
function createAuthScreen() {
    ...
    overlay.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.85); z-index:999999; display:none; flex-direction:column; align-items:center; justify-content:flex-start; direction:rtl; overflow-y:auto; font-family: "Cairo", "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; backdrop-filter: blur(8px); padding: 20px; box-sizing: border-box;';
    
    overlay.innerHTML = `
        <div style="width:100%; max-width:440px; background:#ffffff; border-radius:16px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); border-top: 5px solid #0ea5e9; margin: auto; max-height: 95vh; overflow-y: auto;">
...
// ============================================================================
// 6. نظام الفترات المجانية، والاشتراكات الفردية للمواد للطلاب
// ============================================================================
...
function promptSpecificPayment(amount, title, description, dbUpdateKey, dbUpdateValue) {
    ...
    overlay.innerHTML = `
        <div style="background:#ffffff; width:100%; max-width:400px; border-radius:20px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); text-align:center; padding:35px 25px; border-top: 5px solid #0ea5e9; margin: auto; max-height: 95vh; overflow-y: auto;">
...
async function finishLoginProcess(teacherRef, doc, teacherData, phone, password, currentIP, deviceFingerprint, isAuthorizedAdmin, assignedRole, btn) {
    ...
    // بمجرد الدخول لو كان حسابه Free نعرض التايمر بدل شاشة الحظر
...
// دالة منع كراش زرار تسجيل المعلم وإظهار رسالة تفاعلية
...
// ============================================================================
// 5. الداش بورد المطور (الأصلية كاملة بالتصميم والأنيميشن + التحليلات)
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================================
    // 8. إرسال الطلب للسيرفر والتلخيص
    // ============================================================================
    const processBtn = document.getElementById('process-btn');
    ...
    if (processBtn) {
        processBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                let aiPrompt = "";
                if (isSummaryMode) {
                    aiPrompt = `MANDATORY_STRICT_INSTRUCTION: YOU MUST GENERATE A COMPREHENSIVE, HIGHLY DETAILED STUDY SUMMARY FOR THE ENTIRE MATERIAL. DO NOT GENERATE EXAM QUESTIONS. **CRUCIAL: ADAPT THE TONE, EXPLANATION STYLE, AND COMPLEXITY TO PERFECTLY MATCH THE EGYPTIAN MINISTRY OF EDUCATION AND AL-AZHAR CURRICULUM FOR "${yearText}". IF PRIMARY STAGE: USE SIMPLE, CLEAR, AND ENGAGING LANGUAGE. IF PREPARATORY STAGE: USE INFORMATIVE AND STRUCTURED LANGUAGE. IF SECONDARY STAGE: USE ACADEMIC, THOROUGH, AND MATURE LANGUAGE.** OUTPUT AS JSON ARRAY 'qa_data' WHERE EACH ITEM REPRESENTS A SECTION. 1) USE 'q' AS THE SECTION TITLE. 2) USE 'a' FOR THE FULL EXPLANATION. 3) SET 'type' TO 'ESSAY'. STRICTLY ADHERE TO THE UPLOADED MATERIAL ONLY.`;
                } else {
                    aiPrompt = `MANDATORY_STRICT_INSTRUCTION: 1) YOU MUST RANDOMIZE THE QUESTIONS COMPLETELY EVERY TIME. DO NOT REPEAT PREVIOUSLY GENERATED QUESTIONS. 2) EXAM MUST MATCH EGYPTIAN MINISTRY OF EDUCATION AND AL-AZHAR STANDARDS FOR "${yearText}". FOR PRIMARY: USE SIMPLE LANGUAGE. FOR PREPARATORY: USE STRUCTURED LANGUAGE. FOR SECONDARY: USE COMPLEX, ACADEMIC QUESTIONS TESTING HIGHER ORDER THINKING. 3) ESSAY QUESTIONS MUST INCLUDE (بم تفسر, ماذا يحدث إذا, ما النتائج المترتبة على, قارن بين). 4) FOR EVERY TRUE/FALSE QUESTION, YOU MUST PROVIDE A DETAILED SCIENTIFIC REASON. 5) FOR EVERY MCQ QUESTION, YOU MUST PROVIDE A DETAILED REASON. 6) YOU MUST GENERATE A MINIMUM OF 20 QUESTIONS AND MAXIMUM OF 50 QUESTIONS FOR EACH SECTION. TOTAL MINIMUM 60 QUESTIONS. STRICTLY ADHERE TO THE UPLOADED MATERIAL ONLY.`;
                }
    ...
    // ============================================================================
    // 9. عرض المخرجات وتوليد ملف PDF شامل بطريقة احترافية
    // ============================================================================
    ...
    // ============================================================================
    // 12. نظام "المعلم التفاعلي" الأصلي (الروبوت البشري في واجهة الشات الصغيرة)
    // ============================================================================
    ...
    // فتح وقفل الروبوت القديم (تم تحويله ليفتح الشاشة الغامرة الجديدة أسفله)
    // تم الاحتفاظ بالكود الأصلي هنا كـ Fallback
        ...
        try {
            const response = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({
                    ...
                    strict_prompt_command: "MANDATORY_STRICT_INSTRUCTION: YOU ARE A FRIENDLY, HUMAN-LIKE EXPERT TEACHER ROBOT. ANSWER THE STUDENT'S QUESTION IN ARABIC. BE ENCOURAGING, CLEAR, AND CONCISE. ACT AS A HUMAN."
                })
            });
            ...
            let finalReply = data.answer || data.reply || data.message || "أنا هنا لمساعدتك! ولكن يبدو أن السيرفر الخلفي لم يتم تفعيله للدردشة المباشرة حتى الآن. 😊";
    ...
    // ============================================================================
    // إضافات الشاشة الغامرة (Immersive 3D Modal) اللي اتكلمنا عليها
    // ============================================================================
