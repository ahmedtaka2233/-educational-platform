          ...
          snapshot.docChanges().forEach(change => {
              if (change.type === "added" || change.type === "modified") {
                  ...
                  if (data.paymentRequestedAt) {
                      ...
                      // نظهر الإشعار فقط للطلبات التي تمت خلال آخر 5 دقائق لتجنب الإزعاج عند الريفريش
...
async function handleUserLogin() {
    ...
    try {
        ...
        if (isAuthorizedAdmin) {
            ...
            // فحص إذا كان الأدمن يمتلك باسوورد مسبق أم لا (لو الباسوورد 1234 أو مش موجود، يوجهه لإنشائه)
            ...
            if (!hasAdminPass) {
                openAdminCreatePasswordModal(async (newPassword) => {
                    ...
                    // حفظ الباسورد الجديد وتفعيل الأدمن
...
async function finishLoginProcess(teacherRef, doc, teacherData, phone, password, currentIP, deviceFingerprint, isAuthorizedAdmin, assignedRole, btn) {
    if (doc.exists) {
        if (!isAuthorizedAdmin) {
            ...
            if (teacherData.studentPassword) {
                ...
                // حفظ الرقم السري للطالب في حال لم يكن لديه (يتم توجيهه وحفظه في الداتابيز مباشرة)
                await teacherRef.update({ studentPassword: password });
            }

            // التحقق من بصمة الجهاز (منع مشاركة الحساب)
        ...
        // المنع التام لأي حساب قيد المراجعة (Pending_Review) من فتح الداش بورد أو استخدام VIP
        if (!isAuthorizedAdmin && teacherData.status === "Pending_Review") {
            ...
            btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> دخول المنصة';
            return; // إيقاف التنفيذ هنا لمنع فتح المنصة
        }
        ...
        // إذا كان الحساب مفعلاً VIP أو أدمن
        ...
        teacherData = {
            ...
            phone: phone,
            studentPassword: isAuthorizedAdmin ? null : password, // يتم حفظ الرقم السري للطالب هنا في الداتابيز
    ...
    // إذا كان حساب مجاني، نوجهه للدفع
    document.getElementById('auth-user-card').style.display = 'none';
    document.getElementById('auth-payment-card').style.display = 'block';
    btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> دخول المنصة';
}
...
async function handlePaymentRequest() {
    ...
    try {
        ...
        // إرسال رسالة واتساب فورية للإدارة
...
function loginSuccess(phone, role) {
    ...
    // تفعيل مراقب الإشعارات للإدارة فقط
...
function checkAttempts() {
    ...
    if (attempts >= 3) {
        ...
        // توجيه المستخدم فوراً لصفحة الدفع
        document.getElementById('auth-user-card').style.display = 'none';
        document.getElementById('auth-payment-card').style.display = 'block';
        return false;
    }
...
// ============================================================================
// 7. تحميل الصفحة وفتح شاشة تسجيل الدخول تلقائياً عند فتح الموقع
// ============================================================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================================
    // 10. أداة تسجيل أسلوب المعلم (بصمة الشرح المعتمدة)
    // ============================================================================
