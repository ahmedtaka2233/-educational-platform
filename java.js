...
// إخفاء الشاشة الافتتاحية المخصصة بعد التحميل بسلاسة (متناسق مع الأنيميشن السينمائي)
window.addEventListener('load', () => {
    setTimeout(() => {
        ...
        if (customSplash) {
            ...
            setTimeout(() => customSplash.remove(), 600);
        }
    }, 2500); 
});
...
document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") {
        ...
        // (المدة دي معمولة عشان لو فتح الكاميرا يصور الدرس، التطبيق ميعملش ريستارت ويمسح الصورة)
...
const MULTI_DEVICE_PHONES = [
    "01026336159", // الأدمن
    "01010482432", // العائلة / مشرف
    "01011974537", // العائلة / مشرف
    "01021059077", // العائلة / مشرف
    "01022750575"  // العائلة / مشرف
];
...
// ============================================================================
// 4. رسائل النجاح، وبناء واجهة المستخدم بعد الدخول
// ============================================================================
...
// ============================================================================
// 5. الداش بورد المطور (لوحة تحكم الإدارة)
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (processBtn) {
        processBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // التعديل الخاص بفرض العشوائية التامة والأسباب العلمية الإجبارية
                const serverPayload = {
                    action: 'analyze',
    ...
    // ============================================================================
    // 9. عرض المخرجات وتوليد ملف PDF شامل
    // ============================================================================
    function showOutput(serverData, subjectName) {
        ...
        resultHtml += '<h4 style="color: #0f172a; margin-top: 0; border-bottom: 2px solid #0ea5e9; display: inline-block; padding-bottom: 5px;">تم تلخيص ومعالجة المادة: ' + stripParentheses(serverData.grade || "") + ' - ' + stripParentheses(subjectName) + '</h4>';
        ...
        resultHtml += '<button id="native-print-btn" class="download-pdf-btn" style="background:#0ea5e9; margin-bottom: 10px;"><i class="fas fa-file-pdf"></i> تحميل / حفظ التلخيص كملف PDF على التليفون</button></div>';
    ...
    function preparePDFDOM(serverData, subjectName) {
        ...
        document.getElementById('pdf-header-title').innerText = 'ملف المادة | ' + stripParentheses(serverData.subjectTitle || subjectName) + ' | ' + stripParentheses(serverData.grade || "");
        ...
        if (footerElement) {
            footerElement.innerHTML = 'تاريخ الإنشاء: ' + pdfCreationDate + ' | عدد الأسئلة المستخرجة: ' + questionCount + '<br>تم التوليد بواسطة منصة الذكاء الاصطناعي © 2026';
        }
    }

    // ============================================================================
    // 10. أداة تسجيل أسلوب المعلم
    // ============================================================================
    ...
    // ============================================================================
    // 11. نظام تحويل الموقع إلى تطبيق (PWA) مع نافذة التثبيت
    // ============================================================================
    let deferredPrompt;
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
        ...
        setTimeout(() => {
            if(!document.getElementById('elegant-install-modal')) {
                const installModal = document.createElement('div');
                installModal.id = 'elegant-install-modal';
                installModal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(10, 17, 40, 0.85); z-index:999999999; display:flex; justify-content:center; align-items:center; font-family:"Cairo", sans-serif; backdrop-filter: blur(8px); padding:20px; opacity:0; transition: opacity 0.4s ease; direction:rtl;';
                
                installModal.innerHTML = `
                    <div style="background:#ffffff; width:100%; max-width:380px; border-radius:24px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); overflow:hidden; text-align:center; transform: translateY(20px); transition: transform 0.4s ease; border-top: 5px solid #0ea5e9;">
                        <div style="padding: 40px 20px 20px; background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); position: relative;">
                            <button id="close-elegant-install" style="position:absolute; top:15px; left:15px; background:none; border:none; color:#cbd5e1; font-size:1.5rem; cursor:pointer;"><i class="fas fa-times"></i></button>
                            <img src="1234.jpg" alt="App Icon" style="width: 90px; height: 90px; border-radius: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); border: 3px solid #ffffff; margin-bottom: 15px; object-fit: cover;">
                            <h3 style="color:#ffffff; margin:0; font-size:1.4rem;">Educational platform</h3>
                            <p style="color:#94a3b8; font-size:0.95rem; margin:5px 0 0 0;">النسخة الرسمية</p>
                        </div>
                        <div style="padding: 30px 25px;">
                            <p style="color:#475569; font-size:1.05rem; line-height:1.6; margin-bottom:25px; font-weight:bold;">احصل على تجربة أسرع وأفضل! قم بتثبيت التطبيق على جهازك للوصول المباشر بنقرة واحدة بدون متصفح.</p>
                            <button id="btn-elegant-install" style="width:100%; background:linear-gradient(to right, #0ea5e9, #3b82f6); color:white; border:none; padding:16px; border-radius:12px; font-size:1.15rem; font-weight:bold; cursor:pointer; box-shadow:0 10px 20px rgba(14, 165, 233, 0.3); transition: transform 0.2s, box-shadow 0.2s; margin-bottom:12px;"><i class="fas fa-download"></i> تثبيت التطبيق الآن</button>
                            <button id="btn-elegant-later" style="width:100%; background:#f1f5f9; color:#64748b; border:none; padding:14px; border-radius:12px; font-size:1rem; font-weight:bold; cursor:pointer; transition: background 0.2s;">ليس الآن</button>
                        </div>
                    </div>
                `;
                ...
                setTimeout(() => {
                    ...
                    installModal.querySelector('div').style.transform = 'translateY(0)';
                }, 50);

                const closeModal = () => {
                    installModal.style.opacity = '0';
                    installModal.querySelector('div').style.transform = 'translateY(20px)';
                    setTimeout(() => installModal.remove(), 400);
                };

                document.getElementById('close-elegant-install').onclick = closeModal;
                document.getElementById('btn-elegant-later').onclick = closeModal;

                document.getElementById('btn-elegant-install').onclick = async () => {
                    closeModal();
                    if (deferredPrompt) {
