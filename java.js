...
// النسخة الأصلية (100% كاملة) + دعم PWA Offline + الداش بورد كاملة + المعلم التفاعلي 3D + محاكي الامتحان التفاعلي
// ============================================================================
...
ALL MCQs AND TRUE/FALSE MUST HAVE DETAILED REASONS. THE TONE MUST BE 100% IDENTICAL TO OFFICIAL EGYPTIAN EXAMS. DO NOT DEVIATE.`;
    ...
    function showOutput(serverData, subjectName) {
        ...
        if (btnContainer) {
            ...
            if (!isSummaryMode) {
                interactiveExamData = serverData.qa_data.filter(q => q.type === "MCQ" || q.type === "TF");
    ...
    // ============================================================================
    // ميزة الامتحان التفاعلي (Exam Simulation Mode) - Premium Feature
    // ============================================================================
    ...
    async function submitInteractiveExam(isTimeOut = false) {
        ...
        interactiveExamData.forEach((q, index) => {
            ...
            let studentAnswer = selectedRadio ? selectedRadio.value : null;
            let feedbackDiv = document.getElementById(`feedback_${index}`);
            
            let isCorrect = false;
            if (q.type === "MCQ") {
                isCorrect = studentAnswer && (q.a.includes(studentAnswer) || studentAnswer.includes(q.a));
            } else if (q.type === "TF") {
                let correctAnswerStr = q.a.includes("صح") || q.a.includes("true") ? "صح" : "خطأ";
            ...
            feedbackDiv.innerHTML = `
                ...
                ${!isCorrect ? `الإجابة النموذجية: ${q.a}<br>` : ''}
                <div style="margin-top:8px; font-size:0.9rem;"><strong>التفسير العلمي:</strong> ${q.reason || 'لا يوجد تفسير إضافي'}</div>
            `;
        });

        let percentage = Math.round((score / total) * 100);
        let resultMsg = isTimeOut ? "انتهى الوقت! " : "تم التسليم بنجاح! ";
        resultMsg += `نتيجتك: ${score} من ${total} (${percentage}%)`;
        
        showCustomAlert(resultMsg, percentage >= 50 ? 'success' : 'error');
        document.getElementById('submit-interactive-exam-btn').style.display = 'none';

        // حفظ التحليلات ورفعها
        let analyticsRecord = {
    ...
    // ============================================================================
    // نظام "المعلم التفاعلي" الأصلي والغامر
    // ============================================================================
