...
function checkSessionTimeout() {
    if (isVIPLoggedIn) {
        ...
        if (lastActivity && (Date.now() - parseInt(lastActivity) > SESSION_TIMEOUT_MS)) {
            logout();
            showCustomAlert("تم تسجيل الخروج التلقائي لحماية حسابك بسبب عدم التفاعل لمدة 10 دقائق. يرجى تسجيل الدخول مجدداً.", 'error');
        }
    }
}
    ...
    if (processBtn) {
        processBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // ----------------------------------------------------------------------------
                // البرومبت الشامل الذكي (مطابق 100% لمواصفات وزارة التربية والتعليم المصرية 2026)
                // ----------------------------------------------------------------------------
...
ALL MCQs AND TRUE/FALSE MUST HAVE DETAILED REASONS. THE TONE MUST BE 100% IDENTICAL TO OFFICIAL EGYPTIAN EXAMS. DO NOT DEVIATE.`;
                ...
                const serverPayload = {
                    ...
                    min_questions_per_section: isSummaryMode ? 5 : 15,
                    max_questions_per_section: isSummaryMode ? 20 : 40,
                    question_counts: {
                        mcq: isSummaryMode ? 0 : 15,
                        true_false: isSummaryMode ? 0 : 15,
                        essay: isSummaryMode ? 10 : 15,
                        strict_min_per_section: isSummaryMode ? 5 : 15,
                        strict_max_per_section: isSummaryMode ? 20 : 40
                    },
    ...
    if (tutorFabBtn && tutorChatWin && closeTutorBtn) {
        tutorFabBtn.addEventListener('click', () => {
            if (tutorChatWin.style.display === 'none' || tutorChatWin.style.opacity === '0') {
                tutorChatWin.style.display = 'flex';
                setTimeout(() => {
                    tutorChatWin.style.transform = 'scale(1)';
                    tutorChatWin.style.opacity = '1';
                }, 10);
            } else {
        ...
        try {
            const response = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({
                    ...
                    strict_prompt_command: "MANDATORY_STRICT_INSTRUCTION: YOU ARE A FRIENDLY, HUMAN-LIKE EXPERT TEACHER ROBOT FOR THE EGYPTIAN CURRICULUM. ANSWER THE STUDENT'S QUESTION IN ARABIC. BE ENCOURAGING, CLEAR, AND CONCISE."
                })
            });
